...
func (p reader) fetchPlan(ctx context.Context, id uuid.UUID) (*workflow.Plan, error) {
	...
	res, err := p.plansClient.ReadItem(ctx, partitionKey, id.String(), itemOpt)
	if err != nil {
	...
	// }
	return convertToPlan(res)
}

func convertToPlan(response *azcosmos.ItemResponse) (*workflow.Plan, error) {
	var err error
	var resp plansEntry
